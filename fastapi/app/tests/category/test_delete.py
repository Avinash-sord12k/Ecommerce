...
async def test_delete_nonexistent_category(client: AsyncClient):
    ...
    response = await client.delete(f"/api/v1/category/{non_existent_category_id}")
    response_json = response.json()
    logger.debug(response_json)
...
@pytest.mark.asyncio(loop_scope="session")
async def test_delete_already_deleted_category(client: AsyncClient, category_data: dict):
    # Create category
    response = await client.post("/api/v1/category/create", json=category_data)
    response_json = response.json()
    logger.debug(response_json)
