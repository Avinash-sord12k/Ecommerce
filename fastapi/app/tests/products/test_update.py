...
async def test_update_product(
    client: AsyncClient, product: str, product_data: dict
):
    product_id = product
    product_data["name"] = "Updated Product Name"

    response = await client.put(
        f"/api/v1/product/update/{product_id}", json=product_data
    )
...
async def test_update_product_without_product_data(
    client: AsyncClient, product: str
):
    product_id = product
    response = await client.put(f"/api/v1/product/update/{product_id}")
    response_json = response.json()
    logger.debug(response_json)
...
@pytest.mark.asyncio(loop_scope="session")
async def test_update_product_with_invalid_id(client: AsyncClient):
