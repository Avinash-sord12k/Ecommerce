...
@pytest.mark.asyncio(loop_scope="session")
async def test_get_product_by_invalid_category_id(client: AsyncClient):
...
@pytest.mark.asyncio(loop_scope="session")
async def test_get_product_by_non_existent_category_id(client: AsyncClient):
...
async def test_get_product_by_category_id_without_category_id(
    ...
    response = await client.get("/api/v1/product/get-by-category-id")
    response_json = response.json()
    logger.debug(response_json)
...
@pytest.mark.asyncio(loop_scope="session")
async def test_get_product_by_category_id_without_id(client: AsyncClient):
    response = await client.get("/api/v1/product/get-by-category-id")
    response_json = response.json()
    logger.debug(response_json)
    assert response.status_code == HTTP_405_METHOD_NOT_ALLOWED
